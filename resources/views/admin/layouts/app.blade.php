            ...
            <header class="h-16 bg-white border-b border-slate-200 flex items-center justify-between px-6 shadow-sm">
                ...
                <div class="relative" x-data="{ open: false }" @click.outside="open = false">
                    ...
                    <div x-show="open" x-cloak x-transition:enter="transition ease-out duration-150"
                        ...
                        class="absolute right-0 mt-2 w-80 bg-white rounded-xl shadow-lg ring-1 ring-slate-200 z-50 overflow-hidden">
                            ...
                            @if ($unreadContacts->isNotEmpty())
                                ...
                                @foreach ($unreadContacts as $contact)
                                    <a href="{{ route('admin.contacts.index') }}" @click="open = false"
                                        class="flex items-start gap-3 px-4 py-3 hover:bg-slate-50 transition-colors border-b border-slate-50 last:border-0">
                                        <div
                                            class="w-9 h-9 rounded-full bg-cyan-100 flex items-center justify-center shrink-0 mt-0.5">
                                        ...
                                        <div class="flex-1 min-w-0">
                                            ...
                                            <p class="text-xs text-cyan-600 mt-0.5">Pesan baru
                            ...
                            @if ($unreadComments->isNotEmpty())
                                ...
                                @foreach ($unreadComments as $comment)
                                    <a href="{{ route('admin.komentar.index') }}" @click="open = false"
                                        class="flex items-start gap-3 px-4 py-3 hover:bg-slate-50 transition-colors border-b border-slate-50 last:border-0">
                                        <div
                                            class="w-9 h-9 rounded-full bg-emerald-100 flex items-center justify-center shrink-0 mt-0.5">
                            ...
                            @if ($pendingUsers->isNotEmpty())
                                ...
                                @foreach ($pendingUsers as $pendingUser)
                                    <a href="{{ route('admin.users.index') }}" @click="open = false"
                                        class="flex items-start gap-3 px-4 py-3 hover:bg-slate-50 transition-colors border-b border-slate-50 last:border-0">
                                        <div
                                            class="w-9 h-9 rounded-full bg-amber-100 flex items-center justify-center shrink-0 mt-0.5">
                                        ...
                                        <div class="flex-1 min-w-0">
                                            <p class="text-sm font-medium text-slate-800 truncate">
                                                {{ $pendingUser->name }}</p>
                                            <p class="text-xs text-slate-400 truncate">{{ $pendingUser->email }}</p>
                                            <p class="text-xs text-amber-600 mt-0.5">Menunggu
                        ...
                        @if ($totalNotifications > 0)
                            <div class="px-4 py-3 border-t border-slate-100 flex gap-3">
                                @if ($unreadContacts->isNotEmpty())
                                    <a href="{{ route('admin.contacts.index') }}" @click="open = false"
                                        class="flex-1 text-center text-xs font-medium text-cyan-600 hover:underline">
                                        Lihat pesan &rarr;
                                    </a>
                                @endif
                                @if ($unreadComments->isNotEmpty())
                                    <a href="{{ route('admin.komentar.index') }}" @click="open = false"
                                        class="flex-1 text-center text-xs font-medium text-emerald-600 hover:underline">
                                        Lihat komentar &rarr;
                                    </a>
                                @endif
                                @if ($pendingUsers->isNotEmpty())
                                    <a href="{{ route('admin.users.index') }}" @click="open = false"
                                        class="flex-1 text-center text-xs font-medium text-amber-600 hover:underline">
                                        Lihat pengguna &rarr;
                                    </a>
                                @endif
                            </div>
        ...
        <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6"
            ...
            <div class="flex items-start gap-4 mb-5">
                <div :class="{
                    ...
                    <svg x-show="type === 'danger'" class="w-5 h-5 text-red-600" fill="none" viewBox="0 0 24 24"
                        stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                    </svg>
                    <svg x-show="type === 'warning'" class="w-5 h-5 text-amber-600" fill="none"
                        viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
                    </svg>
                    <svg x-show="type === 'success'" class="w-5 h-5 text-green-600" fill="none"
                        viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                    </svg>
