...
@section('content')
    ...
    <div class="space-y-6" x-data="{ open: {{ $errors->hasAny(['name', 'email', 'password', 'role']) ? 'true' : 'false' }} }">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-slate-200">
            <div class="flex items-center justify-between px-6 py-4 border-b border-slate-200">
                <div class="flex items-center gap-3">
                    <h2 class="text-base font-semibold text-slate-800">Menunggu Persetujuan</h2>
                        ...
                        <div class="flex items-center justify-between px-6 py-4">
                            <div class="flex items-center gap-4">
                                ...
                                <div>
                                    ...
                                    <p class="text-xs text-slate-400 mt-0.5">Daftar {{ $user->created_at->diffForHumans() }}
                                    </p>
                                </div>
                            </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-slate-200">
            <div class="flex items-center justify-between px-6 py-4 border-b border-slate-200">
                <h2 class="text-base font-semibold text-slate-800">Pengguna Aktif</h2>
                @can('users.create')
                        ...
                        @forelse ($approved as $user)
                            <tr class="hover:bg-slate-50 transition-colors">
                                <td class="px-6 py-3">
                                    <div class="flex items-center gap-3">
                                        <div
                                            class="w-8 h-8 rounded-full bg-cyan-100 flex items-center justify-center shrink-0">
                                ...
                                <td class="px-6 py-3 text-slate-400 text-xs">{{ $user->created_at->format('d M Y') }}</td>
                                <td class="px-6 py-3 text-right">
                                    <div class="flex items-center justify-end gap-2">
                                        {{-- Edit --}}
                                        @if (Auth::user()->is_admin)
                                            <a href="{{ route('admin.users.edit', $user) }}"
                                                class="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-cyan-700 bg-cyan-50 hover:bg-cyan-100 rounded-lg transition-colors">
                                        ...
                                        @if (Auth::user()->adminTier() < $user->adminTier())
                                            <form method="POST" action="{{ route('admin.users.toggle', $user) }}" x-data
                                                ...
                                                @csrf @method('PATCH')
                                                <button type="submit"
                                                    class="{{ $user->is_active ? 'text-red-600 bg-red-50 hover:bg-red-100' : 'text-green-600 bg-green-50 hover:bg-green-100' }} px-3 py-1.5 text-xs font-semibold rounded-lg transition-colors">
        ...
        <div x-show="open" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4"
            ...
            <div class="relative bg-white rounded-xl shadow-xl w-full max-w-lg">
                <div class="flex items-center justify-between px-6 py-4 border-b border-slate-200">
                    <h3 class="text-sm font-semibold text-slate-800">Tambah Pengguna</h3>
                ...
                <form method="POST" action="{{ route('admin.users.store') }}" class="p-6 space-y-4">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="block text-sm font-medium text-slate-700 mb-1.5">Password <span
                                    class="text-red-500">*</span></label>
                            <input type="password" name="password"
                                class="w-full rounded-lg border border-slate-300 bg-white text-slate-800 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500 @error('password') border-red-500 @enderror">
                            @error('password')
                                <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                            @enderror
                        </div>
                        <div>
                            ...
                            <select name="role"
                                class="w-full rounded-lg border border-slate-300 bg-white text-slate-800 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500 @error('role') border-red-500 @enderror">
