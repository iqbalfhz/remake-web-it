        ...
        <form method="POST" action="{{ route('admin.artikel.update', $artikel) }}" enctype="multipart/form-data"
            ...
            <div>
                ...
                <textarea name="excerpt" rows="2"
                    class="w-full rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-800 dark:text-white px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500 @error('excerpt') border-red-500 @enderror">{{ old('excerpt', $artikel->excerpt) }}</textarea>
                @error('excerpt')
                    <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <textarea name="content" rows="10"
                    class="w-full rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-800 dark:text-white px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500 font-mono @error('content') border-red-500 @enderror">{{ old('content', $artikel->content) }}</textarea>
                @error('content')
                    <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                @if ($artikel->image)
                    <div class="mb-2">
                        <img src="{{ $artikel->image_url }}" alt="" class="h-24 w-auto rounded-lg object-cover">
