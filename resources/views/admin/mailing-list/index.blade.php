...
@section('content')
    ...
    <div x-data="{ open: {{ $errors->any() ? 'true' : 'false' }} }"
        ...
        <div class="flex items-center justify-between px-6 py-4 border-b border-slate-200">
            <h2 class="text-base font-semibold text-slate-800">Mailing List</h2>
                ...
                <tbody class="divide-y divide-slate-200">
                    @forelse ($mailingLists as $item)
                        <tr class="hover:bg-slate-50 transition-colors">
                            <td class="px-6 py-3 font-medium text-slate-800">{{ $item->department }}</td>
                            <td class="px-6 py-3 text-slate-600">
                                <a href="mailto:{{ $item->email }}"
                                    class="hover:text-cyan-600 transition-colors">{{ $item->email }}</a>
                            </td>
        ...
        <div x-show="open" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4"
            ...
            <div class="relative bg-white rounded-xl shadow-xl w-full max-w-md">
                <div class="flex items-center justify-between px-6 py-4 border-b border-slate-200">
                    <h3 class="text-sm font-semibold text-slate-800">Tambah Mailing List</h3>
