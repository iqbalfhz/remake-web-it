    ...
    <div class="space-y-4">
        ...
        <div
            ...
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h2 class="text-base font-semibold text-slate-800">Komentar</h2>
                    <p class="text-xs text-slate-400">{{ $comments->total() }} komentar dari pembaca</p>
                </div>
            </div>
        </div>
                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse ($comments as $komentar)
                        <tr class="hover:bg-slate-50 transition-colors" x-data>
                            <td class="px-6 py-4">
                                <div class="font-medium text-slate-800">{{ $komentar->name }}</div>
                                <a href="mailto:{{ $komentar->email }}"
                                    class="text-xs text-cyan-500 hover:underline">{{ $komentar->email }}</a>
                            </td>
                            <td class="px-6 py-4 max-w-xs">
                                <p class="text-slate-600 line-clamp-2">{{ $komentar->body }}</p>
                            </td>
                            <td class="px-6 py-4">
                                @if ($komentar->article)
                                    <a href="{{ route('artikel.show', $komentar->article) }}" target="_blank"
                                        class="text-cyan-600 hover:underline line-clamp-1 text-xs">
